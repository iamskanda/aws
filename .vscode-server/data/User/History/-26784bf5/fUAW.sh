...
directory_path="/home/ec2-user/myscript/test"

# Change to the specified directory
# cd "$directory_path" || exit

# Find files that were last modified more than 4 weeks ago
old_files=$(find . -type f -mtime +28)

# Check if there are any old files
if [ -n "$old_files" ]; then
    # Display a warning message
    echo "Deleting old files in $directory_path"

    # Sort files by size in descending order
    files_to_delete=$(du -h "$old_files" | sort -rh | cut -f2)
    ...
    echo "No old files found in $directory_path"
fi
